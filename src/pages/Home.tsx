    ...
    <div className="group bg-surface-container-low rounded-xl overflow-hidden transition-all duration-300 hover:translate-y-[-8px] hover:bg-surface-container-high">
      <Link to={`/product/${product.id}`} className="block">
        <div className="aspect-[4/3] overflow-hidden relative">
          ...
          {product.tags?.includes("hot") && (
            <div className="absolute top-3 left-3 bg-primary px-3 py-1 rounded-sm text-[10px] font-bold uppercase tracking-widest text-on-primary shadow-lg z-10">
              Hot
            </div>
          )}
          {product.tags?.includes("bestseller") && (
            <div className="absolute top-3 right-3 bg-secondary px-3 py-1 rounded-sm text-[10px] font-bold uppercase tracking-widest text-on-secondary shadow-lg z-10">
              Bestseller
            </div>
          )}
        </div>
      ...
      <div className="p-6">
        <Link to={`/product/${product.id}`}>
          <h3 className="text-lg font-bold mb-1 line-clamp-2 min-h-[3rem] hover:text-primary transition-colors leading-tight">
            {product.name}
          </h3>
        </Link>
...
export default function Home() {
  ...
  // Get bestseller products and limit to 4
  ...
  return (
    <div className="bg-surface text-on-surface">
        ...
        <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          <div className="z-10 text-center lg:text-left">
            ...
            <p className="text-xl text-on-surface-variant mb-8 max-w-lg">
              Nhanh – Ngon – Giá tốt, đặt món chỉ trong 1 click ngay khi đang
              combat.
            </p>
            <div className="flex flex-col sm:flex-row gap-4 justify-center lg:justify-start">
              <Link
                to="/menu"
                className="bg-primary text-on-primary px-10 py-4 rounded-full text-lg font-extrabold hover:shadow-[0_0_20px_rgba(255,141,140,0.4)] transition-all flex items-center justify-center"
              >
                Đặt Ngay
              </Link>
            </div>
      ...
      {/* Featured Products (Only show bestseller products) */}
      <section className="py-24 px-6 max-w-7xl mx-auto">
        <div className="flex justify-between items-end mb-12">
          <div>
            ...
            <p className="text-on-surface-variant">
              Top các vật phẩm bán chạy nhất
            </p>
          </div>
          <Link
            to="/menu"
            className="hidden md:flex items-center gap-1 text-primary font-bold hover:brightness-125 transition-all text-sm uppercase tracking-widest"
          >
            Xem tất cả{" "}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {loading ? (
            ...
            <div className="col-span-full text-center py-10 text-on-surface-variant bg-surface-container-low rounded-xl">
              Chưa có vật phẩm Bestseller nào được tìm thấy.
            </div>
          )}
        </div>
      </section>
            ...
            {loading
              ? Array(3)
                  ...
                  .map((_, i) => (
                    <div
                      key={i}
                      className="bg-surface-container-highest p-8 rounded-2xl animate-pulse space-y-4"
                    >
